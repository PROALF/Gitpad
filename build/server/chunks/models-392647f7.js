import { H as HF_ACCESS_TOKEN, M as MODELS, d as OLD_MODELS } from './private-26a5baf7.js';
...
const combinedEndpoint = endpoint.transform((data) => {
  if (data.host === "tgi" || data.host === void 0) {
...
const modelsRaw = z.array(
  z.object({
    ...
    webSearchSummaryPromptTemplate: z.string().default(
      "{{userMessageToken}}{{answer}}{{userMessageEndToken}}{{userMessageToken}}The text above should be summarized to best answer the query: {{query}}.{{userMessageEndToken}}{{assistantMessageToken}}Summary: "
    ),
    webSearchQueryPromptTemplate: z.string().default(
      "{{userMessageToken}}The following messages were written by a user, trying to answer a question.{{userMessageEndToken}}{{#each messages}}{{#ifUser}}{{@root.userMessageToken}}{{content}}{{@root.userMessageEndToken}}{{/ifUser}}{{/each}}{{userMessageToken}}What plain-text english sentence would you input into Google to answer the last question? Answer with a short (10 words max) simple sentence.{{userMessageEndToken}}{{assistantMessageToken}}Query: "
    ),
...
export { defaultModel as d, models as m, oldModels as o, validateModel as v };
//# sourceMappingURL=models-392647f7.js.map
